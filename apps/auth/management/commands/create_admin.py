...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--username", default=os.environ.get("DJANGO_ADMIN_USER", "admin"))
    ...
    def handle(self, *args, **options):
        ...
        if not password:
            raise CommandError(
                "Password is required. Pass --password or set DJANGO_ADMIN_PASSWORD env var."
            )
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created admin user '{username}' with email '{email}'.")
            )
        else:
            self.stdout.write(
